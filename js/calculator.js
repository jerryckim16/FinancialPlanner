function calculate() {
  ...
  var contributions = getNormalizedContributions();
  var invCount = investments.length;
  var monthlyRates = investments.map(function (inv) {
    var effective = (inv.rate || 0) - (inv.expenseRatio || 0);
    return effective / 100 / 12;
  });
  ...
  for (var k = 0; k < invCount; k++) {
    var initBal = investments[k].initialPosition || 0;
    invBalances.push(initBal);
    invContributions.push(initBal);
    invDividendsPaid.push(0);
    startingPortfolio += initBal;
  }
  var investment = startingPortfolio;
  var totalContributions = startingPortfolio;
  ...
  for (var m = 1; m <= months; m++) {
    ...
    if (invCount > 0) {
      for (var k = 0; k < invCount; k++) {
        var thisContrib = investContrib * contributions[k];
  ...
  setText("netWorthValue", formatUSD(finalNetWorth));
  setText("totalAssets", formatUSD(finalAssets));
  var assetParts = [];
  ...
  setText("assetsSub", assetParts.join(" + "));
  ...
  if (!hadDebt && finalDeficitDebt <= 0.01) {
    setText("debtFree", "No debt");
    setText("debtInterestPaid", deficitEverOccurred ? "Deficit recovered within period" : "");
  } else if (finalDeficitDebt > 0.01) {
    setText("debtFree", "In deficit");
    setText("debtInterestPaid", formatUSD(finalDeficitDebt) + " accrued at " + (DEFICIT_APR * 100) + "% APR");
  } else if (debtFreeMonth !== null) {
    setText("debtFree", formatMonths(debtFreeMonth));
    setText("debtInterestPaid", formatUSD(totalInterestPaidOnDebt) + " total interest paid");
  } else {
    setText("debtFree", "Not within " + years + "y");
    setText("debtInterestPaid", formatUSD(totalInterestPaidOnDebt) + " interest paid so far");
  }
  ...
  // Per-investment breakdown table (mirrored on both tabs)
  var breakdownHtml = '';
  if (last.investmentBreakdown && last.investmentBreakdown.length > 1) {
    breakdownHtml = '<div class="breakdown-header">Investment Breakdown</div>';
    breakdownHtml += '<div class="breakdown-table">';
    var anyDividends = last.investmentBreakdown.some(function (b) { return b.dividendsPaid > 0.01; });
    breakdownHtml += '<div class="breakdown-row breakdown-row-header">' +
  ...
  ["investmentBreakdown", "investmentBreakdown2"].forEach(function (id) {
    var el = document.getElementById(id);
  ...
  ["barChart", "barChart2"].forEach(function (chartId) {
    var chart = document.getElementById(chartId);
    if (!chart) return;
    renderChart(chart, filteredData, maxBalance, maxDebt);
  });

  function renderChart(chart, filteredData, maxBalance, maxDebt) {
    chart.innerHTML = "";
